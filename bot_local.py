...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        highest_spent = df_plot.pivot_table(
            index = 'age',
            values = 'spent'
        ).sort_values(by='spent', ascending=False)

        highest_approved_conversion = df_plot.pivot_table(
            index = 'age',
            values = 'approved_conversion'
        ).sort_values(by='approved_conversion', ascending=False)

        highest_cpc = df_plot.pivot_table(
            index = 'age',
            values = 'cpc'
        ...
        plot_info = list(zip(
            ...
            [
                highest_spent.tail(1).index[0], highest_approved_conversion.tail(1).index[0], 
                highest_cpc.tail(1).index[0]
            ]
        ))
